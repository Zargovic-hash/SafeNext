...
const TableView = ({ filteredRegulations, isFullscreen, handleStartAudit }) => {
  ...
  const commonHeaderProps = "px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider cursor-pointer select-none transition-all duration-300 hover:text-blue-600 hover:bg-blue-50/50 backdrop-blur-sm";
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-white/95 via-white/90 to-blue-50/80 backdrop-blur-lg rounded-3xl border border-gray-200/50 shadow-2xl overflow-hidden"
      style={{
        backgroundImage: `
    ...
    >
      <div className={`overflow-x-auto ${isFullscreen ? 'max-h-[calc(100vh-120px)]' : 'max-h-[calc(100vh-140px)]'} overflow-y-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400`}>
            ...
            <tr>
              <th className={`${commonHeaderProps} w-40`} onClick={() => requestSort('domaine')}>
                <div className={commonHeaderStyle}>
                  <span className="group-hover:text-blue-700 transition-colors">Domaine</span>
                  <span className="text-blue-500 opacity-70 group-hover:opacity-100 transition-opacity">{getSortIndicator('domaine')}</span>
                </div>
              </th>
              <th className={`${commonHeaderProps} min-w-[200px]`} onClick={() => requestSort('titre')}>
                <div className={commonHeaderStyle}>
                  <span className="group-hover:text-blue-700 transition-colors">Titre</span>
                  <span className="text-blue-500 opacity-70 group-hover:opacity-100 transition-opacity">{getSortIndicator('titre')}</span>
                </div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider min-w-[400px]">Exigence</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider min-w-[400px]">Références légales</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider min-w-[400px]">Documents légaux</th>              
              <th className={`${commonHeaderProps} w-32`} onClick={() => requestSort('conformite')}>
                <div className={commonHeaderStyle}>
                  <span className="group-hover:text-blue-700 transition-colors">Statut</span>
                  <span className="text-blue-500 opacity-70 group-hover:opacity-100 transition-opacity">{getSortIndicator('conformite')}</span>
                </div>
              </th>
              <th className={`${commonHeaderProps} w-32`} onClick={() => requestSort('prioritée')}>
                <div className={commonHeaderStyle}>
                  <span className="group-hover:text-blue-700 transition-colors">Priorité</span>
                  <span className="text-blue-500 opacity-70 group-hover:opacity-100 transition-opacity">{getSortIndicator('prioritée')}</span>
                </div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider min-w-[300px]">Plan d'actions</th>
              <th className={`${commonHeaderProps} w-40`} onClick={() => requestSort('owner')}>
                <div className={commonHeaderStyle}>
                  <span className="group-hover:text-blue-700 transition-colors">Propriétaire</span>
                  <span className="text-blue-500 opacity-70 group-hover:opacity-100 transition-opacity">{getSortIndicator('owner')}</span>
                </div>
              </th>
              <th className={`${commonHeaderProps} w-32`} onClick={() => requestSort('deadline')}>
                <div className={commonHeaderStyle}>
                  <span className="group-hover:text-blue-700 transition-colors">Échéance</span>
                  <span className="text-blue-500 opacity-70 group-hover:opacity-100 transition-opacity">{getSortIndicator('deadline')}</span>
                </div>
              </th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider w-32">Actions</th>
            </tr>
          </thead>
